  ...
  try {
    ...
    if (onlineAgents.length === 0) {
      ...
      if (moved) {
        ...
        // Agents may be stationary because they've reached their destination — check if ticker code exists
        const { execSync } = require('child_process');
        const hasTickerCode = execSync('grep -c "WALK_TICK_MS" /home/kai/projects/botmesh/state/src/index.js').toString().trim() !== '0';
        hasTickerCode
          ? pass('Walk ticker — agents at destinations (ticker code confirmed present)')
